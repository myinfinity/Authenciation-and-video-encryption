...
*******************************************************************************/
uint32_t ETH_Init(ETH_InitTypeDef* ETH_InitStruct, uint16_t PHYAddress)//PHYAddress 0x01
{
    uint32_t tmpreg = 0;
    uint16_t RegValue = 0;
...
//    rt_kprintf("ETH_macmiiar:%x!\n", ETH->MACMIIAR);
    /* reset phy */
    ETH_WritePHYRegister(PHYAddress, PHY_BCR, PHY_Reset); //复位
    tickstart= rt_tick_get_millisecond();
    do{
        asm("nop");      /* waiting for finish */
    }while((rt_tick_get_millisecond()-tickstart)<1000);

//    rt_kprintf("PHY_BCR:%x!\n", ETH_ReadPHYRegister(PHYAddress, PHY_BCR));

    tickstart= rt_tick_get_millisecond();
    RegValue = ((uint16_t)0x8000);////////////////////这里我修改了，因为原代码while循环根本不会进入，不能检查是否reset了
    while( (RegValue&(PHY_Reset)) != 0)//但是这里还是有问题，就算没有复位，regvalue还是会被返回为0，一样的
    {//不过根据打印的regvalue值 1100，自动协商+全双工 ，证明复位应该是成功了。懒得改了
...
//        rt_kprintf("RegValue:%x!\n", RegValue);
        if((rt_tick_get_millisecond()-tickstart)>10000)
        {
            ...
            rt_kprintf("Wait phy reset timeout!");
            while(1);
        }
    }

//    rt_kprintf("PHY_BCR:%x!\n", ETH_ReadPHYRegister(PHYAddress, PHY_BCR));
    /* waiting for link up *///这里是在读取链接状态,第2位linkStatus
    tickstart= rt_tick_get_millisecond();
    RegValue = 0;
    while((RegValue&(PHY_Linked_Status)) == 0)
    {
       RegValue = ETH_ReadPHYRegister(PHYAddress, PHY_BSR);//PHYAddress 0x01
//       rt_kprintf("RegValue:%x!\n", RegValue);
//       rt_kprintf("PHY_BCR:%x!\n", ETH_ReadPHYRegister(PHYAddress, PHY_BCR));
       if((rt_tick_get_millisecond()-tickstart)>100000)
       {
           LOG_E("Error:Wait phy linking timeout!\r\n");
           rt_kprintf("Wait phy linking timeout!");
           while(1);
       }
    }
    ...
    while((RegValue&PHY_AutoNego_Complete) == 0)
    {
        ...
        if((rt_tick_get_millisecond()-tickstart)>10000)
        {
            ...
            rt_kprintf("Wait phy auto-negotiation complete timeout!");
            while(1);
        }
    }
    ...
    else if(( RegValue & 0x0030 ) == 0x0020)
    {
        ...
        rt_kprintf("Link speed:1000Mbps.\n");
    }
#endif
...
//    uint32_t tickstart=0;
    ...
    //输入是多少M？
    //目前比较清晰了 首先之前mac不初始化是由于时钟由PB1供应，但PB1没接
    //其次千兆网口使用内部时钟是倍频不到125的所以做不了。

    ETH_InitTypeDef ETH_InitStructure={0};
    RCC_PLL3Cmd(DISABLE);
//    RCC_PREDIV2Config(RCC_PREDIV2_Div1);
//    RCC_PLL3Config(RCC_PLL3Mul_16);
...
//    rt_thread_delay(100);
    ...
    {rt_kprintf("ETH->DMABMR:%08x!\n", ETH->DMABMR);}
    LOG_D("ETH RST Finish\r\n");
    rt_kprintf("ETH RST Finish\r\n");
...
static int rt_hw_ch32_eth_init(void)
{
    ...
    ch32v30x_eth_device.dev_addr[2] = 0xe6;
...
//test software demo
//#include "ch32v30x.h"
//void check_pb1_clk(void)
//{
//    uint32_t clk_status = RCC->CFGR2; // 读取时钟配置
//    uint32_t eth_clk = RCC->CTLR;     // 读取时钟状态
//
//    rt_kprintf("RCC_CFGR2: 0x%08X\n", clk_status);
//    rt_kprintf("RCC_CTLR:  0x%08X\n", eth_clk);
//}


//void check_mac_clock(void) //好像不管用
//{
//    uint32_t prev = ETH->PTPSSIR;
//    rt_thread_mdelay(10); // 等待一小段时间
//    uint32_t curr = ETH->PTPSSIR;
//
//    rt_kprintf("PTPSSIR before: 0x%08X\n", prev);
//    rt_kprintf("PTPSSIR after:  0x%08X\n", curr);
//
//    if (prev == curr) {
//        rt_kprintf("Error: No MAC clock detected! Check PB1 input.\n");
//    } else {
//        rt_kprintf("MAC clock detected. PB1 125MHz input is working.\n");
//    }
//}
