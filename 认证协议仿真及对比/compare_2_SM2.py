...
def point_subtract(P, Q,  p):
    # 计算-Q的坐标
    if Q == '0':
        return P
    Q_neg = (Q[0], (-Q[1]) % p)
    # 调用点加法函数
    return add_point(P, Q_neg, p)
...
def protocol():
    ...
    p, a, b, h, G, n = args         # 序列解包
    s = creat_number(512)
    Ppub = mult_point(G, s, p, a)
    print("Ppub = sG = ", Ppub)

    #节点
    ID = creat_number(512)
    SA = creat_number(512)
    R1 = mult_point(G, SA, p, a)
    print("R = SAG = ", R1)
    IDB = creat_number(512)
    SB = creat_number(512)
    R1B = mult_point(G, SB, p, a)
    print("R = SBG = ", R1B)

    #注册方
    m = creat_number(512)
    n = creat_number(512)
    R2 = mult_point(G, m, p, a)
    R2B = mult_point(G, n, p, a)
    R = add_point(R1, R2, p)
    RB = add_point(R1B, R2B, p)
    print("RB = R1B + R2B = ", RB)
    print("R = R1 + R2 = ", R)
    h = sm3_hash(int(str(ID) + point_to_bits(R)))
    hB = sm3_hash(int(str(IDB) + point_to_bits(RB)))
    print("h = sm3_hash(ID || R) = ", h)
    print("hB = sm3_hash(IDB || RB) = ", hB)
    d = m + h * s
    dB = n + hB * s
    print("d = m + h * s = ", d)
    print("dB = n + hB * s = ", dB)
    ...
    #节点
    check1 = mult_point(G,d+SA,p,a)
    check1B = mult_point(G,dB+SB,p,a)
    print("check1 = dG + lG = ", check1)
    print("check1B = dBG + lB = ", check1B)
    check2 = add_point(R,mult_point(Ppub,h,p,a),p)
    chech2B = add_point(RB,mult_point(Ppub,hB,p,a),p)
    print("check2 = R + hPpub = ", check2)
    print("check2B = RB + hBPub = ", chech2B)

    #认证流程(A)
    xa = creat_number(512)
    ya = creat_number(512)

    YA = mult_point(G,ya,p,a)
    print("YA = yaG = ", YA)
    XA = mult_point(G,xa,p,a)
    print("XA = xaG = ", XA)
    eA = sm3_hash(str(XA)+str(ID)+str(R)+str(YA))
    print("eA = sm3_hash(XA || ID || R || YA) = ", eA)
    zA = ya + eA*(d+SA)
    print("zA = ya + eA*(d+SA) = ", zA)

    #认证流程(B)
    h1 = sm3_hash(str(ID)+point_to_bits(R))
    mid1 = add_point(mult_point(Ppub,eA*h1,p,a),mult_point(R,eA,p,a),p)
    
    YA1 = point_subtract(mult_point(G,zA,p,a) ,mid1,p)
    print("YA1 = zAG - h1Ppub - eAR = ", YA1)
    eA1 = sm3_hash(str(XA)+str(ID)+str(R)+str(YA1))
    print("eA1 = sm3_hash(XA || ID || R || YA1) = ", eA1)


    xb = creat_number(512)
    yb = creat_number(512)
    YB = mult_point(G,yb,p,a)
    print("YB = ybG = ", YB)
    XB = mult_point(G,xb,p,a)
    print("XB = xbG = ", XB)
    eB = sm3_hash(str(XB)+str(IDB)+str(RB)+str(YB))
    print("eB = sm3_hash(XB || IDB || RB || YB) = ", eB)
    zB = yb + eB*(dB+SB)
    print("zB = yb + eB*(dB+SB) = ", zB)

    KAB = sm3_hash(point_to_bits(mult_point(XA,xb,p,a)))
    print("KAB = sm3_hash(XA * xb) = ", KAB)
    hB = sm3_hash(str(IDB)+point_to_bits(RB))
    mid2 = add_point(mult_point(Ppub,eB*hB,p,a),mult_point(RB,eB,p,a),p)
    YB1 = point_subtract(mult_point(G,zB,p,a) ,mid2,p)
    print("YB1 = zBG - hBPub - eBR = ", YB1)
    eB1 = sm3_hash(str(XB)+str(IDB)+str(RB)+str(YB1))
    print("eB1 = sm3_hash(XB || IDB || RB || YB1) = ", eB1)
    KAB1 = sm3_hash(point_to_bits(mult_point(XB,xa,p,a)))
    print("KAB1 = sm3_hash(XB * xa) = ", KAB1)
