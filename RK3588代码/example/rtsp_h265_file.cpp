...
int H265File::ReadFrame(char* in_buf, int in_buf_size, bool* end)
{
	...
	*end = false;
	...
	for (i=0; i<bytes_read-6; i++) {
		...
		uint8_t nal_type = ((uint8_t)m_buf[i+start_code] >> 1) & 0x3F;
		if ((nal_type <= 31) && (((uint8_t)m_buf[i+start_code+2] & 0x80) == 0x80)) {
			is_find_start = true;
			i += 4;
			break;
		}
